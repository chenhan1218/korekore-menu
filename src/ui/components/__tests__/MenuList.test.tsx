...
describe('MenuList', () => {
  ...
  it('calls onVariantSelect when variant is selected', async () => {
    ...
    const firstVariantButton = allVariantButtons.find(btn =>
      btn.textContent?.includes('單點') && btn.textContent?.includes('500')
    ) as HTMLButtonElement
    await user.click(firstVariantButton)

    expect(onVariantSelect).toHaveBeenCalledWith('item_1', expect.objectContaining({
      spec: '單點',
      price: 500,
      tax_type: '稅込'
    }))
